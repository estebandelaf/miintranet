...
/**
 * BaseNoticia para trabajo con un objeto de la tabla noticia
 * Noticias para ser publicada en la portada de la app y en rss
 * Esta clase entrega los métodos básicos para ser extendida por Noticia
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseNoticia implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla noticia
	public $id; ///< ID de la noticia: integer(32) NOT NULL DEFAULT 'nextval('noticia_id_seq'::regclass)' AUTO PK 
	public $titulo; ///< Tí­tulo de la noticia: character varying(30) NOT NULL DEFAULT '' 
	public $cuerpo; ///< Texto de la noticia: text() NOT NULL DEFAULT '' 
	public $fechahora; ///< Fecha y hora en la que se creo la noticia: timestamp without time zone() NOT NULL DEFAULT '' 
	public $expiracion; ///< Fecha hasta cuando la noticia es válida: date() NOT NULL DEFAULT '' 
	public $usuario_id; ///< ID del usuario que publica la noticia: integer(32) NOT NULL DEFAULT '' FK:usuario.id
	...
	public $resumen; ///< Resumen de la noticia para ser mostrado en los links hacia esta: character varying(80) NOT NULL DEFAULT '' 


	/**
	 * Constructor de la clase BaseNoticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseNoticia, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseNoticia mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['id']) && $arreglo['id']!='')
			$this->id = $arreglo['id'];
		if(isset($arreglo['titulo']) && $arreglo['titulo']!='')
			$this->titulo = $arreglo['titulo'];
		if(isset($arreglo['cuerpo']) && $arreglo['cuerpo']!='')
			$this->cuerpo = $arreglo['cuerpo'];
		if(isset($arreglo['fechahora']) && $arreglo['fechahora']!='')
			$this->fechahora = $arreglo['fechahora'];
		if(isset($arreglo['expiracion']) && $arreglo['expiracion']!='')
			$this->expiracion = $arreglo['expiracion'];
		if(isset($arreglo['usuario_id']) && $arreglo['usuario_id']!='')
			$this->usuario_id = $arreglo['usuario_id'];
		if(isset($arreglo['resumen']) && $arreglo['resumen']!='')
			$this->resumen = $arreglo['resumen'];

	}

	/**
	 * Recupera las columnas de una fila desde la tabla noticia, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrNoticia = self::$bd->getFila("SELECT * FROM noticia WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if($arrNoticia) {
			foreach($arrNoticia as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrNoticia = null;
	...
	/**
	 * Verifica si la fila asociada a la PK (id) existe en la tabla noticia
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM noticia WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseNoticia de la tabla noticia
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM noticia WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseNoticia en la tabla noticia
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: id, titulo, cuerpo, fechahora, expiracion, usuario_id, audit_programa, audit_usuario, audit_fechahora, resumen
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseNoticia en la tabla noticia
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO noticia (
				...
				".($this->titulo!==null?"'".self::$bd->proteger($this->titulo)."'":'NULL').",
				".($this->cuerpo!==null?"'".self::$bd->proteger($this->cuerpo)."'":'NULL').",
				".($this->fechahora!==null?"'".self::$bd->proteger($this->fechahora)."'":'NULL').",
				".($this->expiracion!==null?"'".self::$bd->proteger($this->expiracion)."'":'NULL').",
				".($this->usuario_id!==null?"'".self::$bd->proteger($this->usuario_id)."'":'NULL').",
				".($this->resumen!==null?"'".self::$bd->proteger($this->resumen)."'":'NULL')."
				, '".AUDIT_PROGRAMA."'
				, '".AUDIT_USUARIO."'
				, NOW()
			)
	...
	/**
	 * Actualiza un objeto del tipo BaseNoticia de la tabla noticia
	...
	final private function update () {
		...
		self::$bd->consulta("
			UPDATE noticia
			SET
				titulo=".($this->titulo!==null?"'".self::$bd->proteger($this->titulo)."'":'NULL').",
				cuerpo=".($this->cuerpo!==null?"'".self::$bd->proteger($this->cuerpo)."'":'NULL').",
				fechahora=".($this->fechahora!==null?"'".self::$bd->proteger($this->fechahora)."'":'NULL').",
				expiracion=".($this->expiracion!==null?"'".self::$bd->proteger($this->expiracion)."'":'NULL').",
				usuario_id=".($this->usuario_id!==null?"'".self::$bd->proteger($this->usuario_id)."'":'NULL').",
				resumen=".($this->resumen!==null?"'".self::$bd->proteger($this->resumen)."'":'NULL')."
	...
	/**
	 * Recupera un objeto de tipo Usuario asociado al objeto BaseNoticia
	 * Se requiere que ya se haya usado BaseNoticia->get()
...
/**
 * BaseNoticias para trabajo con listado de objetos de la tabla noticia
 * Noticias para ser publicada en la portada de la app y en rss
 * Esta clase entrega los métodos básicos para ser extendida por Noticias
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseNoticias implements Bases {
	...
	/**
	 * Constructor de la clase BaseNoticias
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseNoticias
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseNoticias, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla noticia por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseNoticia desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM noticia';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM noticia';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM noticia';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM noticia';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseNoticia desde la tabla noticia,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM noticia';
		else $query = 'SELECT * FROM noticia';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objNoticia->$columna = $valor;
					array_push($objetos, $objNoticia);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseNoticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla noticia
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla noticia
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla noticia
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla noticia
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla noticia
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
