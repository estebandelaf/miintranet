...
/**
 * BaseProducto_proveedor para trabajo con un objeto de la tabla producto_proveedor
 * Relación entre productos y los proveedores que los ofrecen
 * Esta clase entrega los métodos básicos para ser extendida por Producto_proveedor
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseProducto_proveedor implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla producto_proveedor
	public $producto_id; ///< Código del producto: character varying(20) NOT NULL DEFAULT '' PK FK:producto.id
	public $proveedor_id; ///< Rut del proveedor: integer(32) NOT NULL DEFAULT '' PK FK:proveedor.id
	public $valor; ///< Valor de compra por unidad definida en el producto: integer(32) NOT NULL DEFAULT '' 
	...
	/**
	 * Constructor de la clase BaseProducto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseProducto_proveedor, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseProducto_proveedor mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['proveedor_id']) && $arreglo['proveedor_id']!='')
			$this->proveedor_id = $arreglo['proveedor_id'];
	...
	/**
	 * Recupera las columnas de una fila desde la tabla producto_proveedor, seteando
	 * los valores de los atributos de la clase y además devolviendo un arreglo
	 * con dichos valores
	 * Se requiere que los campos de la PK (producto_id, proveedor_id) esten establecidos
	 * @return Array Arreglo con fila de la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrProducto_proveedor = self::$bd->getFila("SELECT * FROM producto_proveedor WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND proveedor_id=".($this->proveedor_id!==null?"'".self::$bd->proteger($this->proveedor_id)."'":'NULL')."");
		if($arrProducto_proveedor) {
			foreach($arrProducto_proveedor as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrProducto_proveedor = null;
		}
		return $arrProducto_proveedor;
	}

	/**
	 * Verifica si la fila asociada a la PK (producto_id, proveedor_id) existe en la tabla producto_proveedor
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM producto_proveedor WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND proveedor_id=".($this->proveedor_id!==null?"'".self::$bd->proteger($this->proveedor_id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseProducto_proveedor de la tabla producto_proveedor
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM producto_proveedor WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND proveedor_id=".($this->proveedor_id!==null?"'".self::$bd->proteger($this->proveedor_id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseProducto_proveedor en la tabla producto_proveedor
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: producto_id, proveedor_id, valor, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseProducto_proveedor en la tabla producto_proveedor
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO producto_proveedor (
				...
				".($this->proveedor_id!==null?"'".self::$bd->proteger($this->proveedor_id)."'":'NULL').",
	...
	/**
	 * Actualiza un objeto del tipo BaseProducto_proveedor de la tabla producto_proveedor
	 * No es posible actualizar la PK (producto_id, proveedor_id)
	 * Campos que no pueden ser null: producto_id, proveedor_id, valor, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	...
	final private function update () {
		...
		self::$bd->consulta("
			...
			SET
				...
				, audit_fechahora = NOW()
			WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND proveedor_id=".($this->proveedor_id!==null?"'".self::$bd->proteger($this->proveedor_id)."'":'NULL')."
		");
	...
	/**
	 * Recupera un objeto de tipo Producto asociado al objeto BaseProducto_proveedor
	 * Se requiere que ya se haya usado BaseProducto_proveedor->get()
	...
	/**
	 * Recupera un objeto de tipo Proveedor asociado al objeto BaseProducto_proveedor
	 * Se requiere que ya se haya usado BaseProducto_proveedor->get()
	 * @return Proveedor Objeto de tipo Proveedor con datos seteados o null en caso de que no existe la asociación
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	public function getProveedor () {
		require(DIR.'/class/db/final/Proveedor.class.php');
		$objProveedor = new Proveedor();
		$objProveedor->set(array('id'=>$this->proveedor_id));
		if($objProveedor->exist()) {
			$objProveedor->get();
			return $objProveedor;
		}
		return null;
...
/**
 * BaseProducto_proveedors para trabajo con listado de objetos de la tabla producto_proveedor
 * Relación entre productos y los proveedores que los ofrecen
 * Esta clase entrega los métodos básicos para ser extendida por Producto_proveedors
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseProducto_proveedors implements Bases {
	...
	/**
	 * Constructor de la clase BaseProducto_proveedors
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseProducto_proveedors
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseProducto_proveedors, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla producto_proveedor por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseProducto_proveedor desde la base de datos,
	 ...
	 */
		...
		$query = 'SELECT COUNT(*) FROM producto_proveedor';
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM producto_proveedor';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM producto_proveedor';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM producto_proveedor';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM producto_proveedor';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseProducto_proveedor desde la tabla producto_proveedor,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM producto_proveedor';
		else $query = 'SELECT * FROM producto_proveedor';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objProducto_proveedor->$columna = $valor;
					array_push($objetos, $objProducto_proveedor);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseProducto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla producto_proveedor
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla producto_proveedor
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla producto_proveedor
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla producto_proveedor
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla producto_proveedor
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
