...
/**
 * BaseEnlace_categoria para trabajo con un objeto de la tabla enlace_categoria
 * Categorí­as de los enlaces generales de la aplicación
 * Esta clase entrega los métodos básicos para ser extendida por Enlace_categoria
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseEnlace_categoria implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla enlace_categoria
	public $id; ///< ID de la categorí­a: integer(32) NOT NULL DEFAULT 'nextval('enlace_categoria_id_seq'::regclass)' AUTO PK 
	public $nombre; ///< Nombre de la categorí­a: character varying(40) NOT NULL DEFAULT '' 
	public $orden; ///< Order en que serán mostradas las categorías: smallint(16) NOT NULL DEFAULT '99' 
	...
	/**
	 * Constructor de la clase BaseEnlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Destructor de la clase Enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseEnlace_categoria, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseEnlace_categoria mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['orden']) && $arreglo['orden']!='')
			$this->orden = $arreglo['orden'];

	}

	/**
	 * Recupera las columnas de una fila desde la tabla enlace_categoria, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrEnlace_categoria = self::$bd->getFila("SELECT * FROM enlace_categoria WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if($arrEnlace_categoria) {
			foreach($arrEnlace_categoria as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrEnlace_categoria = null;
		}
		return $arrEnlace_categoria;
	}

	/**
	 * Verifica si la fila asociada a la PK (id) existe en la tabla enlace_categoria
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM enlace_categoria WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseEnlace_categoria de la tabla enlace_categoria
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM enlace_categoria WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseEnlace_categoria en la tabla enlace_categoria
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: id, nombre, orden, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseEnlace_categoria en la tabla enlace_categoria
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO enlace_categoria (
				...
				".($this->orden!==null?"'".self::$bd->proteger($this->orden)."'":'NULL')."
				, '".AUDIT_PROGRAMA."'
				, '".AUDIT_USUARIO."'
				, NOW()
			)
	...
	/**
	 * Actualiza un objeto del tipo BaseEnlace_categoria de la tabla enlace_categoria
	...
	final private function update () {
		...
		self::$bd->consulta("
			UPDATE enlace_categoria
			SET
				nombre=".($this->nombre!==null?"'".self::$bd->proteger($this->nombre)."'":'NULL').",
				orden=".($this->orden!==null?"'".self::$bd->proteger($this->orden)."'":'NULL')."
...
/**
 * BaseEnlace_categorias para trabajo con listado de objetos de la tabla enlace_categoria
 * Categorí­as de los enlaces generales de la aplicación
 * Esta clase entrega los métodos básicos para ser extendida por Enlace_categorias
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseEnlace_categorias implements Bases {
	...
	/**
	 * Constructor de la clase BaseEnlace_categorias
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseEnlace_categorias
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseEnlace_categorias, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla enlace_categoria por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseEnlace_categoria desde la base de datos,
	 ...
	 */
		...
		$query = 'SELECT COUNT(*) FROM enlace_categoria';
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM enlace_categoria';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM enlace_categoria';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM enlace_categoria';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM enlace_categoria';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseEnlace_categoria desde la tabla enlace_categoria,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM enlace_categoria';
		else $query = 'SELECT * FROM enlace_categoria';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objEnlace_categoria->$columna = $valor;
					array_push($objetos, $objEnlace_categoria);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseEnlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla enlace_categoria
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla enlace_categoria
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla enlace_categoria
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla enlace_categoria
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla enlace_categoria
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
