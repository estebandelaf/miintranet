...
/**
 * BaseUf para trabajo con un objeto de la tabla uf
 * Valores diarios de la UF
 * Esta clase entrega los métodos básicos para ser extendida por Uf
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseUf implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla uf
	public $fecha; ///< Día del valor: date() NOT NULL DEFAULT '' PK 
	public $valor; ///< Valor de la UF: double precision(53) NOT NULL DEFAULT '' 
	...
	/**
	 * Constructor de la clase BaseUf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseUf, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseUf mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		if($clear) $this->clear();
		if(isset($arreglo['fecha']) && $arreglo['fecha']!='')
	...
	/**
	 * Recupera las columnas de una fila desde la tabla uf, seteando
	 * los valores de los atributos de la clase y además devolviendo un arreglo
	 * con dichos valores
	 * Se requiere que los campos de la PK (fecha) esten establecidos
	 * @return Array Arreglo con fila de la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrUf = self::$bd->getFila("SELECT * FROM uf WHERE fecha=".($this->fecha!==null?"'".self::$bd->proteger($this->fecha)."'":'NULL')."");
		if($arrUf) {
	...
	/**
	 * Verifica si la fila asociada a la PK (fecha) existe en la tabla uf
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM uf WHERE fecha=".($this->fecha!==null?"'".self::$bd->proteger($this->fecha)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseUf de la tabla uf
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM uf WHERE fecha=".($this->fecha!==null?"'".self::$bd->proteger($this->fecha)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseUf en la tabla uf
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: fecha, valor, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseUf en la tabla uf
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO uf (
				...
				".($this->fecha!==null?"'".self::$bd->proteger($this->fecha)."'":'NULL').",
	...
	/**
	 * Actualiza un objeto del tipo BaseUf de la tabla uf
	 * No es posible actualizar la PK (fecha)
	 * Campos que no pueden ser null: fecha, valor, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	...
	final private function update () {
		...
		self::$bd->consulta("
			...
			SET
				...
				, audit_fechahora = NOW()
			WHERE fecha=".($this->fecha!==null?"'".self::$bd->proteger($this->fecha)."'":'NULL')."
		");
...
/**
 * BaseUfs para trabajo con listado de objetos de la tabla uf
 * Valores diarios de la UF
 * Esta clase entrega los métodos básicos para ser extendida por Ufs
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseUfs implements Bases {
	...
	/**
	 * Constructor de la clase BaseUfs
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseUfs
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseUfs, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla uf por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseUf desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM uf';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM uf';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM uf';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM uf';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseUf desde la tabla uf,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM uf';
		else $query = 'SELECT * FROM uf';
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseUf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla uf
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla uf
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla uf
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla uf
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla uf
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
