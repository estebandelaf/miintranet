...
/**
 * BaseEnlace para trabajo con un objeto de la tabla enlace
 * Enlaces generales de la aplicación
 * Esta clase entrega los métodos básicos para ser extendida por Enlace
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseEnlace implements Base {
	...
	public $enlace_categoria_id; ///< Categorí­a del enlace: integer(32) NOT NULL DEFAULT '' FK:enlace_categoria.id
	...
	/**
	 * Constructor de la clase BaseEnlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseEnlace, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseEnlace mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['enlace_categoria_id']) && $arreglo['enlace_categoria_id']!='')
			$this->enlace_categoria_id = $arreglo['enlace_categoria_id'];

	}

	/**
	 * Recupera las columnas de una fila desde la tabla enlace, seteando
	 * los valores de los atributos de la clase y además devolviendo un arreglo
	 * con dichos valores
	 * Se requiere que los campos de la PK (url) esten establecidos
	 * @return Array Arreglo con fila de la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrEnlace = self::$bd->getFila("SELECT * FROM enlace WHERE url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."");
		if($arrEnlace) {
			foreach($arrEnlace as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrEnlace = null;
	...
	/**
	 * Verifica si la fila asociada a la PK (url) existe en la tabla enlace
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM enlace WHERE url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseEnlace de la tabla enlace
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM enlace WHERE url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseEnlace en la tabla enlace
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: url, nombre, enlace_categoria_id, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseEnlace en la tabla enlace
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO enlace (
				...
				".($this->enlace_categoria_id!==null?"'".self::$bd->proteger($this->enlace_categoria_id)."'":'NULL')."
				, '".AUDIT_PROGRAMA."'
				, '".AUDIT_USUARIO."'
				, NOW()
			)
	...
	/**
	 * Actualiza un objeto del tipo BaseEnlace de la tabla enlace
	 * No es posible actualizar la PK (url)
	 * Campos que no pueden ser null: url, nombre, enlace_categoria_id, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	...
	final private function update () {
		...
		self::$bd->consulta("
			...
			SET
				...
				enlace_categoria_id=".($this->enlace_categoria_id!==null?"'".self::$bd->proteger($this->enlace_categoria_id)."'":'NULL')."
				, audit_programa = '".AUDIT_PROGRAMA."'
				, audit_usuario = '".AUDIT_USUARIO."'
				, audit_fechahora = NOW()
			WHERE url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."
		");
	...
	/**
	 * Recupera un objeto de tipo Enlace_categoria asociado al objeto BaseEnlace
	 * Se requiere que ya se haya usado BaseEnlace->get()
	 * @return Enlace_categoria Objeto de tipo Enlace_categoria con datos seteados o null en caso de que no existe la asociación
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	public function getEnlace_categoria () {
		require(DIR.'/class/db/final/Enlace_categoria.class.php');
		$objEnlace_categoria = new Enlace_categoria();
		$objEnlace_categoria->set(array('id'=>$this->enlace_categoria_id));
		if($objEnlace_categoria->exist()) {
			$objEnlace_categoria->get();
			return $objEnlace_categoria;
		}
		return null;
	}
...
/**
 * BaseEnlaces para trabajo con listado de objetos de la tabla enlace
 * Enlaces generales de la aplicación
 * Esta clase entrega los métodos básicos para ser extendida por Enlaces
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseEnlaces implements Bases {
	...
	/**
	 * Constructor de la clase BaseEnlaces
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseEnlaces
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseEnlaces, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla enlace por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseEnlace desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM enlace';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM enlace';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM enlace';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM enlace';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseEnlace desde la tabla enlace,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM enlace';
		else $query = 'SELECT * FROM enlace';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objEnlace->$columna = $valor;
					array_push($objetos, $objEnlace);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseEnlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla enlace
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla enlace
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla enlace
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla enlace
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla enlace
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
