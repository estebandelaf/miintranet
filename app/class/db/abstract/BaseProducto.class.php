...
/**
 * BaseProducto para trabajo con un objeto de la tabla producto
 * Productos, ya sean materias primas, productos finales o insumos de la empresa
 * Esta clase entrega los métodos básicos para ser extendida por Producto
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseProducto implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla producto
	public $id; ///< Identificador: character varying(20) NOT NULL DEFAULT '' PK 
	public $nombre; ///< Nombre: character varying(30) NOT NULL DEFAULT '' 
	public $producto_categoria_id; ///< Categoría final del producto: integer(32) NOT NULL DEFAULT '' FK:producto_categoria.id
	public $unidad_id; ///< Tipo de unidad a utilizar por el producto: integer(32) NOT NULL DEFAULT '' FK:unidad.id
	public $valor; ///< Valor de venta por unidad del producto: integer(32) NOT NULL DEFAULT '0' 
	...
	/**
	 * Constructor de la clase BaseProducto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseProducto, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseProducto mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['unidad_id']) && $arreglo['unidad_id']!='')
			$this->unidad_id = $arreglo['unidad_id'];
	...
	/**
	 * Recupera las columnas de una fila desde la tabla producto, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrProducto = self::$bd->getFila("SELECT * FROM producto WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if($arrProducto) {
	...
	/**
	 * Verifica si la fila asociada a la PK (id) existe en la tabla producto
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM producto WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseProducto de la tabla producto
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM producto WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseProducto en la tabla producto
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: id, nombre, producto_categoria_id, unidad_id, valor, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseProducto en la tabla producto
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO producto (
				...
				".($this->unidad_id!==null?"'".self::$bd->proteger($this->unidad_id)."'":'NULL').",
	...
	/**
	 * Actualiza un objeto del tipo BaseProducto de la tabla producto
	...
	final private function update () {
		...
		self::$bd->consulta("
			...
			SET
				...
				unidad_id=".($this->unidad_id!==null?"'".self::$bd->proteger($this->unidad_id)."'":'NULL').",
	...
	/**
	 * Recupera un objeto de tipo Producto_categoria asociado al objeto BaseProducto
	 * Se requiere que ya se haya usado BaseProducto->get()
	...
	/**
	 * Recupera un objeto de tipo Unidad asociado al objeto BaseProducto
	 * Se requiere que ya se haya usado BaseProducto->get()
	 * @return Unidad Objeto de tipo Unidad con datos seteados o null en caso de que no existe la asociación
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	public function getUnidad () {
		require(DIR.'/class/db/final/Unidad.class.php');
		$objUnidad = new Unidad();
		$objUnidad->set(array('id'=>$this->unidad_id));
		if($objUnidad->exist()) {
			$objUnidad->get();
			return $objUnidad;
		}
...
/**
 * BaseProductos para trabajo con listado de objetos de la tabla producto
 * Productos, ya sean materias primas, productos finales o insumos de la empresa
 * Esta clase entrega los métodos básicos para ser extendida por Productos
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseProductos implements Bases {
	...
	/**
	 * Constructor de la clase BaseProductos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseProductos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseProductos, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla producto por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseProducto desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM producto';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM producto';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM producto';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM producto';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseProducto desde la tabla producto,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM producto';
		else $query = 'SELECT * FROM producto';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objProducto->$columna = $valor;
					array_push($objetos, $objProducto);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseProducto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla producto
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla producto
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla producto
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla producto
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla producto
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
