...
/**
 * BaseEnlace_usuario para trabajo con un objeto de la tabla enlace_usuario
 * Enlaces personales de cada usuario
 * Esta clase entrega los métodos básicos para ser extendida por Enlace_usuario
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseEnlace_usuario implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla enlace_usuario
	public $usuario_id; ///< ID del usuario que creó el enlace: integer(32) NOT NULL DEFAULT '' PK FK:usuario.id
	public $url; ///< Dirección url completa: character varying(200) NOT NULL DEFAULT '' PK 
	public $nombre; ///< Nombre o descripción del enlace: character varying(60) NOT NULL DEFAULT '' 
	...
	/**
	 * Constructor de la clase BaseEnlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseEnlace_usuario, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseEnlace_usuario mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['url']) && $arreglo['url']!='')
			$this->url = $arreglo['url'];
	...
	/**
	 * Recupera las columnas de una fila desde la tabla enlace_usuario, seteando
	 * los valores de los atributos de la clase y además devolviendo un arreglo
	 * con dichos valores
	 * Se requiere que los campos de la PK (usuario_id, url) esten establecidos
	 * @return Array Arreglo con fila de la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrEnlace_usuario = self::$bd->getFila("SELECT * FROM enlace_usuario WHERE usuario_id=".($this->usuario_id!==null?"'".self::$bd->proteger($this->usuario_id)."'":'NULL')." AND url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."");
		if($arrEnlace_usuario) {
			foreach($arrEnlace_usuario as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrEnlace_usuario = null;
		}
		return $arrEnlace_usuario;
	}

	/**
	 * Verifica si la fila asociada a la PK (usuario_id, url) existe en la tabla enlace_usuario
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM enlace_usuario WHERE usuario_id=".($this->usuario_id!==null?"'".self::$bd->proteger($this->usuario_id)."'":'NULL')." AND url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseEnlace_usuario de la tabla enlace_usuario
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM enlace_usuario WHERE usuario_id=".($this->usuario_id!==null?"'".self::$bd->proteger($this->usuario_id)."'":'NULL')." AND url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseEnlace_usuario en la tabla enlace_usuario
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: usuario_id, url, nombre, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseEnlace_usuario en la tabla enlace_usuario
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO enlace_usuario (
				...
				".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL').",
	...
	/**
	 * Actualiza un objeto del tipo BaseEnlace_usuario de la tabla enlace_usuario
	 * No es posible actualizar la PK (usuario_id, url)
	 * Campos que no pueden ser null: usuario_id, url, nombre, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	...
	final private function update () {
		...
		self::$bd->consulta("
			UPDATE enlace_usuario
			SET
				nombre=".($this->nombre!==null?"'".self::$bd->proteger($this->nombre)."'":'NULL')."
				, audit_programa = '".AUDIT_PROGRAMA."'
				, audit_usuario = '".AUDIT_USUARIO."'
				, audit_fechahora = NOW()
			WHERE usuario_id=".($this->usuario_id!==null?"'".self::$bd->proteger($this->usuario_id)."'":'NULL')." AND url=".($this->url!==null?"'".self::$bd->proteger($this->url)."'":'NULL')."
		");
	...
	/**
	 * Recupera un objeto de tipo Usuario asociado al objeto BaseEnlace_usuario
	 * Se requiere que ya se haya usado BaseEnlace_usuario->get()
...
/**
 * BaseEnlace_usuarios para trabajo con listado de objetos de la tabla enlace_usuario
 * Enlaces personales de cada usuario
 * Esta clase entrega los métodos básicos para ser extendida por Enlace_usuarios
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseEnlace_usuarios implements Bases {
	...
	/**
	 * Constructor de la clase BaseEnlace_usuarios
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseEnlace_usuarios
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseEnlace_usuarios, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla enlace_usuario por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseEnlace_usuario desde la base de datos,
	 ...
	 */
		...
		$query = 'SELECT COUNT(*) FROM enlace_usuario';
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM enlace_usuario';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM enlace_usuario';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM enlace_usuario';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM enlace_usuario';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseEnlace_usuario desde la tabla enlace_usuario,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM enlace_usuario';
		else $query = 'SELECT * FROM enlace_usuario';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objEnlace_usuario->$columna = $valor;
					array_push($objetos, $objEnlace_usuario);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseEnlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla enlace_usuario
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla enlace_usuario
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla enlace_usuario
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla enlace_usuario
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla enlace_usuario
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
