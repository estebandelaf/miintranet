...
/**
 * BaseCargo para trabajo con un objeto de la tabla cargo
 * Cargos del personal de la empresa
 * Esta clase entrega los métodos básicos para ser extendida por Cargo
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseCargo implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla cargo
	public $id; ///< ID del cargo: integer(32) NOT NULL DEFAULT 'nextval('cargo_id_seq'::regclass)' AUTO PK 
	public $glosa; ///< Nombre/descripción del cargo: character varying(45) NOT NULL DEFAULT '' 
	public $area_id; ///< Área a la que pertenece el cargo: integer(32) NOT NULL DEFAULT '' FK:area.id
	public $cardinalidad; ///< Indica cuantos pueden tener el cargo, =0 infinitos: smallint(16) NOT NULL DEFAULT '0' 
	...
	/**
	 * Constructor de la clase BaseCargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseCargo, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseCargo mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['cardinalidad']) && $arreglo['cardinalidad']!='')
			$this->cardinalidad = $arreglo['cardinalidad'];

	}

	/**
	 * Recupera las columnas de una fila desde la tabla cargo, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrCargo = self::$bd->getFila("SELECT * FROM cargo WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if($arrCargo) {
			foreach($arrCargo as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrCargo = null;
	...
	/**
	 * Verifica si la fila asociada a la PK (id) existe en la tabla cargo
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM cargo WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseCargo de la tabla cargo
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM cargo WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseCargo en la tabla cargo
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: id, glosa, area_id, cardinalidad, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseCargo en la tabla cargo
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO cargo (
				...
				".($this->cardinalidad!==null?"'".self::$bd->proteger($this->cardinalidad)."'":'NULL')."
				, '".AUDIT_PROGRAMA."'
				, '".AUDIT_USUARIO."'
				, NOW()
			)
	...
	/**
	 * Actualiza un objeto del tipo BaseCargo de la tabla cargo
	...
	final private function update () {
		...
		self::$bd->consulta("
			...
			SET
				...
				area_id=".($this->area_id!==null?"'".self::$bd->proteger($this->area_id)."'":'NULL').",
				cardinalidad=".($this->cardinalidad!==null?"'".self::$bd->proteger($this->cardinalidad)."'":'NULL')."
	...
	/**
	 * Recupera un objeto de tipo Area asociado al objeto BaseCargo
	 * Se requiere que ya se haya usado BaseCargo->get()
...
/**
 * BaseCargos para trabajo con listado de objetos de la tabla cargo
 * Cargos del personal de la empresa
 * Esta clase entrega los métodos básicos para ser extendida por Cargos
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseCargos implements Bases {
	...
	/**
	 * Constructor de la clase BaseCargos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseCargos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseCargos, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla cargo por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseCargo desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM cargo';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM cargo';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM cargo';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM cargo';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseCargo desde la tabla cargo,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM cargo';
		else $query = 'SELECT * FROM cargo';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objCargo->$columna = $valor;
					array_push($objetos, $objCargo);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseCargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla cargo
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla cargo
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla cargo
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla cargo
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla cargo
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
