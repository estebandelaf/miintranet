...
abstract class BasePermiso_login implements Base {
	...
	/**
	 * Constructor de la clase BasePermiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BasePermiso_login, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BasePermiso_login mediante un arreglo,
	...
	/**
	 * Recupera las columnas de una fila desde la tabla permiso_login, seteando
	 * los valores de los atributos de la clase y además devolviendo un arreglo
	 * con dichos valores
	 * Se requiere que los campos de la PK (recurso) esten establecidos
	 * @return Array Arreglo con fila de la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrPermiso_login = self::$bd->getFila("SELECT * FROM permiso_login WHERE recurso=".($this->recurso!==null?"'".self::$bd->proteger($this->recurso)."'":'NULL')."");
		if($arrPermiso_login) {
			foreach($arrPermiso_login as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrPermiso_login = null;
		}
		return $arrPermiso_login;
	}

	/**
	 * Verifica si la fila asociada a la PK (recurso) existe en la tabla permiso_login
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM permiso_login WHERE recurso=".($this->recurso!==null?"'".self::$bd->proteger($this->recurso)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BasePermiso_login de la tabla permiso_login
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM permiso_login WHERE recurso=".($this->recurso!==null?"'".self::$bd->proteger($this->recurso)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BasePermiso_login en la tabla permiso_login
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: recurso, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BasePermiso_login en la tabla permiso_login
	...
	/**
	 * Actualiza un objeto del tipo BasePermiso_login de la tabla permiso_login
	 * No es posible actualizar la PK (recurso)
	 * Campos que no pueden ser null: recurso, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	...
	final private function update () {
		...
		self::$bd->consulta("
			UPDATE permiso_login
			SET

				, audit_programa = '".AUDIT_PROGRAMA."'
				, audit_usuario = '".AUDIT_USUARIO."'
				, audit_fechahora = NOW()
			WHERE recurso=".($this->recurso!==null?"'".self::$bd->proteger($this->recurso)."'":'NULL')."
		");
...
/**
 * BasePermiso_logins para trabajo con listado de objetos de la tabla permiso_login
 * Recursos que solo requieren al usuario logueado
 * Esta clase entrega los métodos básicos para ser extendida por Permiso_logins
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BasePermiso_logins implements Bases {
	...
	/**
	 * Constructor de la clase BasePermiso_logins
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BasePermiso_logins
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BasePermiso_logins, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla permiso_login por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BasePermiso_login desde la base de datos,
	 ...
	 */
		...
		$query = 'SELECT COUNT(*) FROM permiso_login';
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM permiso_login';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM permiso_login';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM permiso_login';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM permiso_login';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BasePermiso_login desde la tabla permiso_login,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM permiso_login';
		else $query = 'SELECT * FROM permiso_login';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objPermiso_login->$columna = $valor;
					array_push($objetos, $objPermiso_login);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BasePermiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla permiso_login
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla permiso_login
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla permiso_login
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla permiso_login
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla permiso_login
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
