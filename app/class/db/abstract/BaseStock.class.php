...
/**
 * BaseStock para trabajo con un objeto de la tabla stock
 * Niveles de stock actuales
 * Esta clase entrega los métodos básicos para ser extendida por Stock
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseStock implements Base {
	...
	public $nivel; ///< Nivel de stock por unidad de producto: integer(32) NOT NULL DEFAULT '' 
	...
	/**
	 * Constructor de la clase BaseStock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseStock, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseStock mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['nivel']) && $arreglo['nivel']!='')
			$this->nivel = $arreglo['nivel'];

	}

	/**
	 * Recupera las columnas de una fila desde la tabla stock, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrStock = self::$bd->getFila("SELECT * FROM stock WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND bodega_id=".($this->bodega_id!==null?"'".self::$bd->proteger($this->bodega_id)."'":'NULL')." AND area_id=".($this->area_id!==null?"'".self::$bd->proteger($this->area_id)."'":'NULL')."");
		if($arrStock) {
	...
	/**
	 * Verifica si la fila asociada a la PK (producto_id, bodega_id, area_id) existe en la tabla stock
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM stock WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND bodega_id=".($this->bodega_id!==null?"'".self::$bd->proteger($this->bodega_id)."'":'NULL')." AND area_id=".($this->area_id!==null?"'".self::$bd->proteger($this->area_id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseStock de la tabla stock
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM stock WHERE producto_id=".($this->producto_id!==null?"'".self::$bd->proteger($this->producto_id)."'":'NULL')." AND bodega_id=".($this->bodega_id!==null?"'".self::$bd->proteger($this->bodega_id)."'":'NULL')." AND area_id=".($this->area_id!==null?"'".self::$bd->proteger($this->area_id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseStock en la tabla stock
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: producto_id, bodega_id, area_id, nivel, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseStock en la tabla stock
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO stock (
				...
				".($this->nivel!==null?"'".self::$bd->proteger($this->nivel)."'":'NULL')."
				, '".AUDIT_PROGRAMA."'
				, '".AUDIT_USUARIO."'
				, NOW()
			)
	...
	/**
	 * Actualiza un objeto del tipo BaseStock de la tabla stock
	...
	final private function update () {
		...
		self::$bd->consulta("
			UPDATE stock
			SET
				nivel=".($this->nivel!==null?"'".self::$bd->proteger($this->nivel)."'":'NULL')."
	...
	/**
	 * Recupera un objeto de tipo Producto asociado al objeto BaseStock
	 * Se requiere que ya se haya usado BaseStock->get()
	...
	/**
	 * Recupera un objeto de tipo Bodega asociado al objeto BaseStock
	 * Se requiere que ya se haya usado BaseStock->get()
	...
	/**
	 * Recupera un objeto de tipo Area asociado al objeto BaseStock
	 * Se requiere que ya se haya usado BaseStock->get()
...
/**
 * BaseStocks para trabajo con listado de objetos de la tabla stock
 * Niveles de stock actuales
 * Esta clase entrega los métodos básicos para ser extendida por Stocks
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseStocks implements Bases {
	...
	/**
	 * Constructor de la clase BaseStocks
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseStocks
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseStocks, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla stock por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseStock desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM stock';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM stock';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM stock';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM stock';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseStock desde la tabla stock,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM stock';
		else $query = 'SELECT * FROM stock';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objStock->$columna = $valor;
					array_push($objetos, $objStock);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseStock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla stock
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla stock
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla stock
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla stock
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla stock
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
