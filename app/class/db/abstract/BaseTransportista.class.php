...
/**
 * BaseTransportista para trabajo con un objeto de la tabla transportista
 * Transportistas externos o internos que se utilizan para el movimiento de productos
 * Esta clase entrega los métodos básicos para ser extendida por Transportista
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseTransportista implements Base {
	...
	public $id; ///< ID del transportista (RUT sin puntos ni dv): integer(32) NOT NULL DEFAULT '' PK 
	public $razonsocial; ///< Razón social del transportista: character varying(50) NOT NULL DEFAULT '' 
	public $interno; ///< Indica si es un transportista de la empresa (=1) o externo (=0): smallint(16) NOT NULL DEFAULT '1' 
	...
	/**
	 * Constructor de la clase BaseTransportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseTransportista, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseTransportista mediante un arreglo,
	...
	final public function set ($arreglo, $clear = true) {
		...
		if(isset($arreglo['interno']) && $arreglo['interno']!='')
			$this->interno = $arreglo['interno'];

	}

	/**
	 * Recupera las columnas de una fila desde la tabla transportista, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrTransportista = self::$bd->getFila("SELECT * FROM transportista WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if($arrTransportista) {
			foreach($arrTransportista as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrTransportista = null;
		}
		return $arrTransportista;
	}

	/**
	 * Verifica si la fila asociada a la PK (id) existe en la tabla transportista
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM transportista WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseTransportista de la tabla transportista
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM transportista WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseTransportista en la tabla transportista
	 * Inserta si el registro no existe, en caso contrario hace un update
	 * Campos que no pueden ser null: id, razonsocial, interno, audit_programa, audit_usuario, audit_fechahora
	 * @return =true en caso de éxito
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseTransportista en la tabla transportista
	...
	final private function insert () {
		...
		self::$bd->consulta("
			INSERT INTO transportista (
				...
				".($this->interno!==null?"'".self::$bd->proteger($this->interno)."'":'NULL')."
				, '".AUDIT_PROGRAMA."'
				, '".AUDIT_USUARIO."'
				, NOW()
			)
	...
	/**
	 * Actualiza un objeto del tipo BaseTransportista de la tabla transportista
	...
	final private function update () {
		...
		self::$bd->consulta("
			...
			SET
				...
				interno=".($this->interno!==null?"'".self::$bd->proteger($this->interno)."'":'NULL')."
...
/**
 * BaseTransportistas para trabajo con listado de objetos de la tabla transportista
 * Transportistas externos o internos que se utilizan para el movimiento de productos
 * Esta clase entrega los métodos básicos para ser extendida por Transportistas
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseTransportistas implements Bases {
	...
	/**
	 * Constructor de la clase BaseTransportistas
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseTransportistas
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseTransportistas, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla transportista por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseTransportista desde la base de datos,
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM transportista';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM transportista';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM transportista';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM transportista';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseTransportista desde la tabla transportista,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM transportista';
		else $query = 'SELECT * FROM transportista';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objTransportista->$columna = $valor;
					array_push($objetos, $objTransportista);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseTransportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla transportista
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla transportista
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla transportista
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla transportista
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla transportista
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
