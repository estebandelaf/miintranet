...
/**
 * BaseActividad_economica para trabajo con un objeto de la tabla actividad_economica
 * Códigos de actividad económica
 * Esta clase entrega los métodos básicos para ser extendida por Actividad_economica
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseActividad_economica implements Base {

	public static $bd; ///< Objeto de acceso a la base de datos
	
	// Atributos de la clase, corresponden a las columnas
	// en la tabla actividad_economica
	public $id; ///< Código de la actividad: integer(32) NOT NULL DEFAULT '' PK 
	public $glosa; ///< Glosa de la actividad: character varying(100) NOT NULL DEFAULT '' 
	...
	/**
	 * Constructor de la clase BaseActividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Limpia los atributos del objeto BaseActividad_economica, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Setea los atributos del objeto BaseActividad_economica mediante un arreglo,
	...
	/**
	 * Recupera las columnas de una fila desde la tabla actividad_economica, seteando
	 ...
	 * @return Array Arreglo con fila de la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function get () {
		$arrActividad_economica = self::$bd->getFila("SELECT * FROM actividad_economica WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if($arrActividad_economica) {
			foreach($arrActividad_economica as $key=>$value)
				$this->$key = $value;
		} else {
			$this->clear();
			$arrActividad_economica = null;
		}
		return $arrActividad_economica;
	}

	/**
	 * Verifica si la fila asociada a la PK (id) existe en la tabla actividad_economica
	 * @return boolean =true si el objeto existe en la base de datos
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function exist () {
		return (boolean) self::$bd->getValor("SELECT COUNT(*) FROM actividad_economica WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
	}

	/**
	 * Elimina un objeto del tipo BaseActividad_economica de la tabla actividad_economica
	...
	final public function delete () {
		...
		self::$bd->consulta("DELETE FROM actividad_economica WHERE id=".($this->id!==null?"'".self::$bd->proteger($this->id)."'":'NULL')."");
		if(!$this->afterDelete()) return false;
		return true;
	}
	
	/**
	 * Guarda un objeto del tipo BaseActividad_economica en la tabla actividad_economica
	...
	/**
	 * Inserta un nuevo objeto del tipo BaseActividad_economica en la tabla actividad_economica
	...
	/**
	 * Actualiza un objeto del tipo BaseActividad_economica de la tabla actividad_economica
...
/**
 * BaseActividad_economicas para trabajo con listado de objetos de la tabla actividad_economica
 * Códigos de actividad económica
 * Esta clase entrega los métodos básicos para ser extendida por Actividad_economicas
 * Cualquier nuevo código debe ser colocado en la clase final NO en esta
 * @author DeLaF, esteban[at]delaf.cl
 * @version 2011-05-15
 */
abstract class BaseActividad_economicas implements Bases {
	...
	/**
	 * Constructor de la clase BaseActividad_economicas
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __construct ($bd) {
	...
	/**
	 * Destructor de la clase BaseActividad_economicas
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	protected function __destruct () {
	...
	/**
	 * Limpia los atributos del objeto BaseActividad_economicas, excepto $bd
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	final public function clear () {
	 ...
	 * campo del select
	 * @param selectStatement Columna/s que se desea seleccionar de la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * @param groupByStatement Columna/s por la que se desea agrupar la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	 ...
	 * Se "protege" el parametro pasado
	 * @param orderByStatement Columna/s de la tabla actividad_economica por la cual se ordenara y si es ASC o DESC
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Entrega la cantidad de objetos de tipo BaseActividad_economica desde la base de datos,
	 ...
	 */
		...
		$query = 'SELECT COUNT(*) FROM actividad_economica';
	...
	final public function getMax ($campo) {
		$query = 'SELECT MAX('.self::$bd->proteger($campo).') FROM actividad_economica';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getMin ($campo) {
		$query = 'SELECT MIN('.self::$bd->proteger($campo).') FROM actividad_economica';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
        }
	...
	final public function getSum ($campo) {
		$query = 'SELECT SUM('.self::$bd->proteger($campo).') FROM actividad_economica';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	...
	final public function getAvg ($campo) {
		$query = 'SELECT AVG('.self::$bd->proteger($campo).') FROM actividad_economica';
		if ($this->whereStatement) $query .= $this->whereStatement;
		return self::$bd->getValor($query);
	}
	
	/**
	 * Recupera objetos de tipo BaseActividad_economica desde la tabla actividad_economica,
	...
	final private function select ($solicitado) {
		...
		if($this->selectStatement) $query = 'SELECT '.$this->selectStatement.' FROM actividad_economica';
		else $query = 'SELECT * FROM actividad_economica';
			...
			if($solicitado=='objetos') {
				...
				foreach($tabla as &$fila) {
					...
					foreach($fila as $columna => $valor)
						$objActividad_economica->$columna = $valor;
					array_push($objetos, $objActividad_economica);
				}
				return $objetos;
			} else {
				return $tabla;
	 ...
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con objetos de tipo BaseActividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una tabla con las columnas y filas de la tabla actividad_economica
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con filas y columnas de la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una fila con las columnas de la tabla actividad_economica
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con columnas de la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera una columna de la tabla actividad_economica
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Array Arreglo con la columna de la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
	...
	/**
	 * Recupera un valor de la tabla actividad_economica
	 * hará uso del whereStatement si no es null, también de limitStatement,
	 * de orderbyStatement y de selectStatement
	 * @return Mixed Valor solicitado de la tabla actividad_economica
	 * @author DeLaF, esteban[at]delaf.cl
	 * @version 2011-05-15
	 */
