...
/**
* Modificada de la original para tener la capacidad de permitir pasar
* el contenido en xml en vez del archivo :-)
* Esto para no depender del shell_exec()
*/
function parseOds($file, $isFile = true) {
	if($isFile) {
		$tmp = get_tmp_dir();
		...
		$xml = file_get_contents($tmp.'/'.$uid.'/content.xml');
	} else {
		$xml = $file;
	}
	$obj = new ods();
