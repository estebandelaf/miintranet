...
if(isset($_POST['save'])) {
	...
	if(isset($_GET['edit']) && !empty($_GET['usuario_id']) && !empty($_GET['url'])) {
...
// formulario para editar o crear nuevo registro
else if(isset($_GET['new']) || (isset($_GET['edit']) && !empty($_GET['usuario_id']) && !empty($_GET['url']))) {
	...
	if(isset($_GET['edit'])) {
		...
		$link = '?edit&amp;'.'usuario_id='.urlencode($_GET['usuario_id']).'&amp;'.'url='.urlencode($_GET['url']);
	} else $link = '?new';
	...
	if(isset($_GET['edit']) && $isPK) {
		if(in_array('usuario_id', $arrFK)) {
			$objFK = $objEnlace_usuario->getUsuario();
			$text = $objFK->apellido.', '.$objFK->nombre;
		} else $text = $objEnlace_usuario->usuario_id;
		echo Form::text('usuario_id', $text);
	} else if(isset($_GET['new']) || (isset($_GET['edit']) && !$isPK)) echo Form::select(($notNull?MiSiTiO::generar('form/asterisco.html'):'').'usuario_id', 'usuario_id', $objUsuarios->listado(), $objEnlace_usuario->usuario_id, 'ID del usuario que creó el enlace', ($notNull?MiSiTiO::generar('form/classObligatorio.html'):''));
...
else if(isset($_GET['delete']) && !empty($_GET['usuario_id']) && !empty($_GET['url'])) {
	// setear pk
	$arrSet = array();
		$arrSet['usuario_id'] = urldecode($_GET['usuario_id']);
		$arrSet['url'] = urldecode($_GET['url']);
...
// tabla con datos de la tabla enlace_usuario
	...
	$objEnlace_usuarios->setSelectStatement('usuario_id, url, nombre');
	// set where
	if(!empty($_GET['search'])) {
		...
		if(isset($columnas['usuario_id'])&&$columnas['usuario_id']!='') {
			array_push($filtros, "usuario_id = '".Enlace_usuarios::$bd->proteger($columnas['usuario_id'])."'");
	...
	if(!empty($_GET['orderby'])) {
		...
		$objEnlace_usuarios->setOrderByStatement('usuario_id,url');
		$linkOrderBy = 'orderby=usuario_id,url&amp;';
	}
	...
	foreach($objEnlace_usuarios->getObjetos() as $objEnlace_usuario) {
		...
		if($objEnlace_usuario->usuario_id!='') {
			$objFK = $objEnlace_usuario->getUsuario();
		...
		$acciones = array();
		array_push($acciones, Tabla::editar('enlace_usuario?edit&amp;'.'usuario_id='.urlencode($objEnlace_usuario->usuario_id).'&amp;'.'url='.urlencode($objEnlace_usuario->url).''));
		array_push($acciones, Tabla::eliminar('eliminar(\'enlace_usuario\', \''.'usuario_id='.urlencode($objEnlace_usuario->usuario_id).'&amp;'.'url='.urlencode($objEnlace_usuario->url).'\')'));
	...
	array_unshift($tabla, array(
		'usuario_id'=>Tabla::orderby('usuario_id', 'enlace_usuario?page=1&amp;'.$linkWhere.$linkNoPages.'orderby=usuario_id'),
